...
public class ConsumerAgent extends Agent {
	...
	public ConsumerAgent(String typeOfInsurance, String firstCondition, String secondCondition, String thirdCondition, int nBestProviders, AID resultsCollector) {
	...
	public void setup() {
		...
		//
		ServiceProposalRequest serviceProposalRequest = new ServiceProposalRequest(typeOfInsurance, firstCondition, secondCondition, thirdCondition);
		try {
	...
	private class CNetInitWrapper extends WrapperBehaviour {
		...
		public int onEnd() {
			if(--nContracts > 0) {
				...
				// for display: updates resultsCollector with outcomes
				if(resultsCollector != null) {
					...
					System.out.println(getLocalName() +  " contract outcomes " + contractOutcomes.size());
					try {
	...
	private class CNetInit extends ContractNetInitiator {
		...
		protected void handleAllResponses(Vector responses, Vector acceptances) {
					...
					try {
						...
						if(servicePrice < bestServicePrice) {
							...
							if(bestServiceProposalMessage != null) {
								...
								ServiceRating serviceRating = new ServiceRating(new Random().nextInt(5));
								try {
									getContentManager().fillContent(reject, serviceRating);
								} catch (CodecException | OntologyException e) {
									e.printStackTrace();
								}
			...
			if(bestServiceProposalMessage != null) {
				...
				ServiceRating serviceRating = new ServiceRating(new Random().nextInt(5) + 5);
				try {
					getContentManager().fillContent(accept, serviceRating);
				} catch (CodecException | OntologyException e) {
					e.printStackTrace();
				}
		...
		protected void handleFailure(ACLMessage failure) {
			...
			System.out.println(myAgent.getLocalName() + ": contractOutcome.add for handle failure.");
			contractOutcomes.add(new ContractOutcome(ContractOutcome.Value.FAILURE));
		}
		...
		protected void handleInform(ACLMessage inform) {
			...
			System.out.println(myAgent.getLocalName() + ": contractOutcome.add for handle inform.");
			contractOutcomes.add(new ContractOutcome(ContractOutcome.Value.SUCCESS));
		}
