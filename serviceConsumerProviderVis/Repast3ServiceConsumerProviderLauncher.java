...
public class Repast3ServiceConsumerProviderLauncher extends Repast3Launcher {
	...
	private List<Pair> secondConditions;
	private List<Pair> thirdConditions;
	private List<String> typeOfInsurances;
	...
	private void launchAgents() {
		...
		int N_CONSUMERS = N;
		int N_CONSUMERS_FILTERING_PROVIDERS = 0;
		int N_PROVIDERS = N*2;
		...
		try {
			...
			for (int i = 0; i < N_PROVIDERS; i++) {
				String typeOfInsurance = typeOfInsurances.get(random.nextInt(typeOfInsurances.size()));
				double comission = random.nextDouble();
				ProviderAgent pa = new ProviderAgent(typeOfInsurance, comission);
				agentContainer.acceptNewAgent("GoodProvider" + i, pa).start();
				DefaultDrawableNode node = 
						generateNode("GoodProvider" + i, getColorBroker(typeOfInsurance),
			...
			for (int i = 0; i < N_CONSUMERS; i++) {
				String typeOfInsurance = typeOfInsurances.get(random.nextInt(typeOfInsurances.size()));
				String firstCondition = String.valueOf(random.nextInt(80));
				String secondCondition = secondConditions.stream()
						.filter(pair -> pair.getKey() == typeOfInsurance)
						.map(r -> r.getValue())
						.collect(Collectors.toList())
						.get(random.nextInt(secondConditions.stream()
								.filter(pair -> pair.getKey() == typeOfInsurance)
								.collect(Collectors.toList())
								.size()
						));
				String thirdCondition = thirdConditions.stream()
						.filter(pair -> pair.getKey() == typeOfInsurance)
						.map(r -> r.getValue())
						.collect(Collectors.toList())
						.get(random.nextInt(thirdConditions.stream()
								.filter(pair -> pair.getKey() == typeOfInsurance)
								.collect(Collectors.toList())
								.size()
						));

				ConsumerAgent ca = new ConsumerAgent(typeOfInsurance, firstCondition, secondCondition, thirdCondition, N_PROVIDERS, resultsCollectorAID);
				mainContainer.acceptNewAgent("Consumer" + i, ca).start();
				consumers.add(ca);
				DefaultDrawableNode node = 
						generateNode("Consumer" + i, getColor(typeOfInsurance),
	...
	private Color getColor(String typeOfInsurance) {
	...
	private Color getColorBroker(String typeOfInsurance) {
		switch (typeOfInsurance) {
			case "Health":
				return Color.red;
			case "Vehicle":
	...
	private void buildClientParameters(){
		...
		typeOfInsurances = new ArrayList<>();
		secondConditions = new ArrayList<>();
		thirdConditions = new ArrayList<>();

		typeOfInsurances.add("Health");
		typeOfInsurances.add("House");
		typeOfInsurances.add("Vehicle");

		secondConditions.add(new Pair("Health", "High Risk"));
		secondConditions.add(new Pair("Health", "Medium Risk"));
		secondConditions.add(new Pair("Health", "Low Risk"));
		secondConditions.add(new Pair("House", "Apartment"));
		secondConditions.add(new Pair("House", "House"));
		secondConditions.add(new Pair("House", "Farm"));
		secondConditions.add(new Pair("Vehicle", "Car"));
		secondConditions.add(new Pair("Vehicle", "Moto"));
		secondConditions.add(new Pair("Vehicle", "Truck"));

		thirdConditions.add(new Pair("Health", "true"));
		thirdConditions.add(new Pair("Health", "false"));
		thirdConditions.add(new Pair("House", "Urban"));
		thirdConditions.add(new Pair("House", "Rural"));
		thirdConditions.add(new Pair("Vehicle", "1999"));
		thirdConditions.add(new Pair("Vehicle", "2001"));
